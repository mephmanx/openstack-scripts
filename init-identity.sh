...
runuser -l root -c "ipa dnsrecord-add $INTERNAL_DOMAIN_NAME. '$IDENTITY_HOST' --a-ip-address=$IDENTITY_VIP"
...
/usr/bin/ipa user-mod stack --sshpubkey="$(cat /opt/stack/.ssh/id_rsa.pub)"
...
# fulfill the request
ipa cert-request --profile-id=SubCA --principal=HTTP/"$EDGE_ROUTER_NAME"."$INTERNAL_DOMAIN_NAME" /tmp/sub-ca.csr --certificate-out=/tmp/empty_dir/subca.cert
