...
GRUB_CMDLINE_LINUX="resume=/dev/mapper/cs_${DRIVE_NAME_UPDATE}-swap rd.lvm.lv=cs_${DRIVE_NAME_UPDATE}/root rd.lvm.lv=cs_${DRIVE_NAME_UPDATE}/swap ${arch}_iommu=on rhgb quiet splash biosdevname=0 systemd.log_level=0 systemd.show_status=0 rd.plymouth=0 plymouth.enable=0 --log-level=0"
