...
EOF
  ...
  do
    IPS+=(`awk -F'=' '$1 == "IPADDR" {print $2}' $entry`)
    ((ct++))
  done
  ((ct--))
  ...
  while [ $start -le $ct ];
  do
    runuser -l root -c  "touch /etc/sysconfig/network-scripts/ifcfg-eth$start"

if [[ ! -z "${IPS[start]}" ]]
then
#static ip addr
cat > /tmp/eth$start <<EOF
# Generated by parse-kickstart
TYPE="Ethernet"
DEVICE="eth$start"
UUID="5844195f-b0d7-4d15-9442-87d1340cca2$start"
ONBOOT="yes"
IPV6INIT="yes"
IPV6_AUTOCONF="yes"
IPADDR=${IPS[start]}
