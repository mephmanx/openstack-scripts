...
rm -rf /var/log/pfsense-install.log
exec 1>/var/log/pfsense-install.log 2>&1 # send stdout and stderr from rc.local to a log file
#set -x
...
./init.sh
HOSTNAME="$ORGANIZATION-$EDGE_ROUTER_NAME-$(cat /var/log/system_suffix.log)"
echo $HOSTNAME
