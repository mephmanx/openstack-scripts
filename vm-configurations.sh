...
function vm_definitions {
  ...
  case $option in
    ...
    "compute")
        ...
        ## the subtraction at the end is for pfsense & cloudsupport & openldap allocation from available memory
        RESERVED_RAM=$(($(( $INSTALLED_RAM * $RAM_PCT_AVAIL_CLOUD/100 )) - PFSENSE_RAM - OPENLDAP_RAM - CLOUDSUPPORT_RAM))
