...
if [ ! -f "/tmp/pfSense-CE-memstick-ADI-prod.img" ]; then
  ...
  docker run -v /tmp/openstack-scripts/project_config.sh:/env/configuration -v /out:/out -v /var/run/libvirt/libvirt-sock:/var/run/libvirt/libvirt-sock -v /tmp:/tmp -v /dev:/dev -v /root:/root --rm -ti --network=host --privileged "$PFSENSE_CACHE_IMAGE:$PFSENSE_VERSION" dev
  sleep 20;
  rm -rf /tmp/pfSense-CE-memstick-ADI-dev.img
  docker run -v /tmp/openstack-scripts/project_config.sh:/env/configuration -v /out:/out -v /var/run/libvirt/libvirt-sock:/var/run/libvirt/libvirt-sock -v /tmp:/tmp -v /dev:/dev -v /root:/root --rm -ti --network=host --privileged "$PFSENSE_CACHE_IMAGE:$PFSENSE_VERSION" prod
