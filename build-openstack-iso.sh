...
if [ ! -f "/tmp/pfSense-CE-memstick-ADI-prod.img" ]; then
  ...
  if [ -f "$PFSENSE_LIB_CACHE" ]; then
    docker run -v /tmp/openstack-scripts/project_config.sh:/env/configuration -v /var/log:/var/log -v /var/run/libvirt/libvirt-sock:/var/run/libvirt/libvirt-sock -v /tmp:/tmp -v /dev:/dev -v /root:/root --rm -ti --network=host --privileged "$PFSENSE_CACHE_IMAGE:$PFSENSE_VERSION" --cachelibs "$PFSENSE_LIB_CACHE"
  else
    docker run -v /tmp/openstack-scripts/project_config.sh:/env/configuration -v /var/log:/var/log -v /var/run/libvirt/libvirt-sock:/var/run/libvirt/libvirt-sock -v /tmp:/tmp -v /dev:/dev -v /root:/root --rm -ti --network=host --privileged "$PFSENSE_CACHE_IMAGE:$PFSENSE_VERSION" --prepare
    sleep 20;
    rm -rf /tmp/pfSense-CE-memstick-ADI-dev.img
    docker run -v /tmp/openstack-scripts/project_config.sh:/env/configuration -v /var/log:/var/log -v /var/run/libvirt/libvirt-sock:/var/run/libvirt/libvirt-sock -v /tmp:/tmp -v /dev:/dev -v /root:/root --rm -ti --network=host --privileged "$PFSENSE_CACHE_IMAGE:$PFSENSE_VERSION"
  fi
  for i in $(docker images |grep "$PFSENSE_CACHE_IMAGE"|awk '{print $3}');do docker rmi "$i";done
