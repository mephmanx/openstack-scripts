...
echo "virt-install --virt-type kvm --name cloudsupport --memory 16000 --vcpus 2 --disk pool=HP-EXT,size=400,bus=scsi --cdrom /var/tmp/cloudsupport-iso.iso --network bridge=os-int-static,model=rtl8139 --network bridge=os-loc-static,model=rtl8139 --os-variant centos8 --graphics vnc"

eval "virt-install --virt-type kvm --name cloudsupport --memory 16000 --vcpus 2 --disk pool=HP-EXT,size=400,bus=scsi --cdrom /var/tmp/cloudsupport-iso.iso --network bridge=os-int-static,model=rtl8139 --network bridge=os-loc-static,model=rtl8139 --os-variant centos8 --graphics vnc" &
