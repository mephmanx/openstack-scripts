...
#update ip of network card 3 so that it is static in the 192.168.0.XXX range
IP=(`awk -F'=' '$1 == "IPADDR" {print $2}' /etc/sysconfig/network-scripts/ifcfg-eth2`)
octet=`echo "$IP" | cut -d . -f 4`
cn=`echo $octet + 35 | bc`
sed -i "s/IPADDR=$IP/IPADDR=192.168.0.$cn/g" /etc/sysconfig/network-scripts/ifcfg-eth2
